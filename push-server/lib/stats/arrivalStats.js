...
class ArrivalStats {
    ...
    addArrivalInfo(msgId, inc, set = {}) {
        ...
        this.mongo.arrival.findByIdAndUpdate(msgId, data, {upsert: true}, (err, doc) => {
    ...
    addPushAll(msg, ttl) {
        ...
        this.topicOnline.getTopicOnline('noti', (count) => {
            ...
            this.addArrivalInfo(msg.id, {'target_android': count}, data);
        });
    }
    ...
    addPushMany(msg, ttl, sentCount) {
        ...
        this.addArrivalInfo(msg.id, {'target_android': sentCount}, data);
    }

    getRateStatusByType(type, callback) {
        this.mongo.arrival.find({type: type})
            .sort({'timeStart': -1})
            .limit(50)
            .exec((err, docs) => {
                const result = [];
