...
function Stats(redis, port) {
    ...
    logger.verbose("ip file %s %s", ipPath, ip);
...
Stats.prototype.shouldDrop = function () {
    if (this.packetDropThreshold != 0 && this.packetAverage1 && this.packetAverage1 > this.packetDropThreshold) {
        logger.verbose('threshold exceeded dropping packet %d > %d', this.packetAverage1, this.packetDropThreshold);
        this.packetDrop++;
        return true;
    } else {
        return false;
...
Stats.prototype.incr = function (key, timestamp) {
    ...
    logger.verbose("incr %s %s", key, hourKey);
};
...
Stats.prototype.incrby = function (key, timestamp, by) {
    if (by > 0) {
        ...
        logger.verbose("incrby %s %s by %d ", key, hourKey, by);
    }
};
